...
interface QFormMonthPickerProps {
  name: string;
  label: string;
  placeholder?: string;
  tooltip?: string;
...
const { name } = defineProps<QFormMonthPickerProps>();
...
<template>
  <FormItem :field="name" :label="label" :tooltip="tooltip" :disabled="disabled" :help="help" :extra="extra" :rules="rules">
    <Picker mode="month" v-model="value" :placeholder="placeholder" allow-clear style="flex: auto"></Picker>
  </FormItem>
</template>
