...
interface QFormYearPickerProps {
  name: string;
  label: string;
  placeholder?: string;
  tooltip?: string;
...
const { name } = defineProps<QFormYearPickerProps>();
...
<template>
  <FormItem :field="name" :label="label" :tooltip="tooltip" :disabled="disabled" :help="help" :extra="extra" :rules="rules">
    <Picker mode="year" v-model="value" :placeholder="placeholder" allow-clear style="flex: auto"></Picker>
  </FormItem>
</template>
