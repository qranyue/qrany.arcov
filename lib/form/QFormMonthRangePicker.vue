...
interface QFormMonthRangePickerProps {
  name: string;
  label: string;
  placeholder?: string;
  tooltip?: string;
...
const { name } = defineProps<QFormMonthRangePickerProps>();
...
<template>
  <FormItem :field="name" :label="label" :tooltip="tooltip" :disabled="disabled" :help="help" :extra="extra" :rules="rules">
    <RangePicker mode="month" v-model="value" :placeholder="placeholder" allow-clear style="flex: auto"></RangePicker>
  </FormItem>
</template>
