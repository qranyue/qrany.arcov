...
interface QFormQuarterRangePickerProps {
  name: string;
  label: string;
  placeholder?: string;
  tooltip?: string;
...
const { name } = defineProps<QFormQuarterRangePickerProps>();
...
<template>
  <FormItem :field="name" :label="label" :tooltip="tooltip" :disabled="disabled" :help="help" :extra="extra" :rules="rules">
    <RangePicker mode="quarter" v-model="value" :placeholder="placeholder" allow-clear style="flex: auto"></RangePicker>
  </FormItem>
</template>
